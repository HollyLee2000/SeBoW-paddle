            ...
            for i in range(layer_child):  # block
                ...
                if layer != len(self.num_children) - 2:
                    if self.classifier_nodes[layer + 1][i] > 1:
                        ...
                        pros.append(paddle.ones([bs, 1]).cuda() * (paddle.reshape(pre_pro[i], [bs, -1])))
                    pros_return['pro{}_{}'.format(str(layer + 2), str(i))] = pro
